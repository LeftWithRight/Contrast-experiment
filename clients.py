        ...
        if self.is_iid:
            ...
            for i_diff_label in range(10):
                for i_same_label in range(10):
                    shard_image_samelabel['image{}{}'.format(i_diff_label, i_same_label)] = train_data[
                                                                                              i_diff_label * 6000 + i_same_label * shard_size: i_diff_label * 6000 + (
                                                                                                      i_same_label + 1) * shard_size]
                    shard_label_samelabel['label{}{}'.format(i_diff_label, i_same_label)] = train_label[
                                                                                              i_diff_label * 6000 + i_same_label * shard_size: i_diff_label * 6000 + (
                                                                                                      i_same_label + 1) * shard_size]
            ...
            for i_diff_label in range(10):
                for j_diff_label in range(10):
                    shard_image_diff['image{}{}'.format(i_diff_label, j_diff_label)] = total_image_diff[
                                                                                         i_diff_label * 3000 + j_diff_label * 300:i_diff_label * 3000 + (
                                                                                                 j_diff_label + 1) * shard_size]
                    shard_label_diff['label{}{}'.format(i_diff_label, j_diff_label)] = total_label_diff[
                                                                                         i_diff_label * 3000 + j_diff_label * 300:i_diff_label * 3000 + (
                                                                                                 j_diff_label + 1) * shard_size]
            ...
            for i_index in range(10):
                for j_index in range(10):
                    ...
                    local_data, local_label = np.vstack((shard_image_01, shard_image_02)), np.vstack(
                        (shard_label_01, shard_label_02))
                    local_label = np.argmax(local_label, axis=1)
                    # 创建一个客户端
                    someone = client(TensorDataset(torch.tensor(local_data), torch.tensor(local_label)), self.dev)
